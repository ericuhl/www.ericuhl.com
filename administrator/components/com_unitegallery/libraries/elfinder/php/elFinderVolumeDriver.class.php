 ...
 **/
abstract class elFinderVolumeDriverUG {
	...
	public function mount(array $opts) {
		...
		$this->id = $this->driverId.(!empty($this->options['id']) ? $this->options['id'] : elFinderUG::$volumesCnt++).'_';
	...
	public function file($hash) {
		...
		return ($file = $this->stat($path)) ? $file : $this->setError(elFinderUG::ERROR_FILE_NOT_FOUND);
		...
		return $this->setError(elFinderUG::ERROR_FILE_NOT_FOUND);
	}
	...
	public function dir($hash, $resolveLink=false) {
		if (($dir = $this->file($hash)) == false) {
			return $this->setError(elFinderUG::ERROR_DIR_NOT_FOUND);
		}
		...
		return $dir && $dir['mime'] == 'directory' && empty($dir['hidden']) 
			? $dir 
			: $this->setError(elFinderUG::ERROR_NOT_DIR);
	}
	...
	public function scandir($hash) {
		...
		if($dir['read']){
			...
			$result = $this->setError(elFinderUG::ERROR_PERM_DENIED);
	...
	public function mkdir($dst, $name) {
		if ($this->commandDisabled('mkdir')) {
			return $this->setError(elFinderUG::ERROR_PERM_DENIED);
		}
		
		if (!$this->nameAccepted($name)) {
			return $this->setError(elFinderUG::ERROR_INVALID_NAME);
		}
		
		if (($dir = $this->dir($dst)) == false) {
			return $this->setError(elFinderUG::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}
		...
		$stat = $this->stat($dst); 
		if (!empty($stat)) { 
			return $this->setError(elFinderUG::ERROR_EXISTS, $name);
		}
		$this->clearcache();
	...
	public function rename($hash, $name) {
		...
		if (!$this->nameAccepted($name)) {
			return $this->setError(elFinderUG::ERROR_INVALID_NAME, $name);
		}
		...
		if (!empty($file['locked'])) {
			return $this->setError(elFinderUG::ERROR_LOCKED, $file['name']);
		}
	...
	public function duplicate($hash, $suffix='copy') {
		if ($this->commandDisabled('duplicate')) {
			return $this->setError(elFinderUG::ERROR_COPY, '#'.$hash, elFinderUG::ERROR_PERM_DENIED);
		}
		
		if (($file = $this->file($hash)) == false) {
			return $this->setError(elFinderUG::ERROR_COPY, elFinderUG::ERROR_FILE_NOT_FOUND);
		}
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		if ($mime == 'unknown' && $this->mimeDetect == 'internal') {
			$mime = elFinderVolumeDriverUG::mimetypeInternalDetect($name);
		}
		...
		if (!$upload) {
			return $this->setError(elFinderUG::ERROR_UPLOAD_FILE_MIME);
		}

		if ($this->uploadMaxSize > 0 && filesize($tmpname) > $this->uploadMaxSize) {
			return $this->setError(elFinderUG::ERROR_UPLOAD_FILE_SIZE);
		}
		...
		if ($file) { // file exists
			if ($this->options['uploadOverwrite']) {
				if (!$file['write']) {
					...
					return $this->setError(elFinderUG::ERROR_NOT_REPLACE, $name);
				} 
				$this->remove($test);
			} else {
	...
	public function paste($volume, $src, $dst, $rmSrc = false) {
		$err = $rmSrc ? elFinderUG::ERROR_MOVE : elFinderUG::ERROR_COPY;
		
		if ($this->commandDisabled('paste')) {
			return $this->setError($err, '#'.$src, elFinderUG::ERROR_PERM_DENIED);
		}

		if (($file = $volume->file($src, $rmSrc)) == false) {
			return $this->setError($err, '#'.$src, elFinderUG::ERROR_FILE_NOT_FOUND);
		}
		...
		if (($dir = $this->dir($dst)) == false) {
			return $this->setError($err, $errpath, elFinderUG::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}
		
		if (!$dir['write'] || !$file['read']) {
			return $this->setError($err, $errpath, elFinderUG::ERROR_PERM_DENIED);
		}
		...
		if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
			return $rmSrc
				? $this->setError($err, $errpath, elFinderUG::ERROR_LOCKED, $volume->path($test))
				: $this->setError($err, $errpath, elFinderUG::ERROR_PERM_DENIED);
		}
		...
		if ($stat) {
			if ($this->options['copyOverwrite']) {
				...
				if (!$this->isSameType($file['mime'], $stat['mime'])) {
					return $this->setError(elFinderUG::ERROR_NOT_REPLACE, $this->_path($test));
				}
				...
				if (($locked = $this->closestByAttr($test, 'locked', true))) {
					return $this->setError(elFinderUG::ERROR_LOCKED, $this->_path($locked));
				}
				// target is entity file of alias
				if ($volume == $this && ($test == @$file['target'] || $test == $this->decode($src))) {
					return $this->setError(elFinderUG::ERROR_REPLACE, $errpath);
				}
				// remove existed file
				if (!$this->remove($test)) {
					return $this->setError(elFinderUG::ERROR_REPLACE, $this->_path($test));
				}
			} else {
		...
		if ($volume == $this) {
			...
			if ($this->_inpath($destination, $source)) {
				return $this->setError(elFinderUG::ERROR_COPY_INTO_ITSELF, $errpath);
			}
		...
		if (!$this->options['copyTo'] || !$volume->copyFromAllowed()) {
			return $this->setError(elFinderUG::ERROR_COPY, $errpath, elFinderUG::ERROR_PERM_DENIED);
		}
		...
		if ($rmSrc) {
			if ($volume->rm($src)) {
				...
				return $this->setError(elFinderUG::ERROR_MOVE, $errpath, elFinderUG::ERROR_RM_SRC);
			}
		}
		return $this->stat($path);
	}
	...
	public function getContents($hash) {
		...
		if ($file['mime'] == 'directory') {
			return $this->setError(elFinderUG::ERROR_NOT_FILE);
		}
	...
	public function extract($hash) {
		...
		if (!$archiver) {
			return $this->setError(elFinderUG::ERROR_NOT_ARCHIVE);
		}
	...
	public function archive($hashes, $mime) {
		...
		if (!$archiver) {
			return $this->setError(elFinderUG::ERROR_ARCHIVE_TYPE);
		}
		...
		foreach ($hashes as $hash) {
			if (($file = $this->file($hash)) == false) {
				return $this->error(elFinderUG::ERROR_FILE_NOT_FOUND, '#'+$hash);
			}
			if (!$file['read']) {
				return $this->error(elFinderUG::ERROR_PERM_DENIED);
			}
			$path = $this->decode($hash);
			if (!isset($dir)) {
				$dir = $this->_dirname($path);
	...
	public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0) {
		...
		if (!$this->canResize($path, $file)) {
			return $this->setError(elFinderUG::ERROR_UNSUPPORT_TYPE);
		}
	...
	public function rm($hash) {
		return $this->commandDisabled('rm')
			? array(elFinderUG::ERROR_ACCESS_DENIED)
	...
	protected function mimetype($path, $name = '') {
		...
		if ($this->mimeDetect == 'finfo') {
			if ($type = @finfo_file($this->finfo, $path)) {
				...
				if ($ext && preg_match('~^application/(?:octet-stream|(?:x-)?zip)~', $type)) {
					if (isset(elFinderVolumeDriverUG::$mimetypes[$ext])) $type = elFinderVolumeDriverUG::$mimetypes[$ext];
				}
			}
		} elseif ($type == 'mime_content_type') {
			$type = mime_content_type($path);
		} else {
			$type = elFinderVolumeDriverUG::mimetypeInternalDetect($path);
		}
		...
		return $type == 'unknown' && $this->mimeDetect != 'internal'
			? elFinderVolumeDriverUG::mimetypeInternalDetect($path)
			: $type;
	...
	static protected function mimetypeInternalDetect($path) {
		...
		return isset(elFinderVolumeDriverUG::$mimetypes[$ext]) ? elFinderVolumeDriverUG::$mimetypes[$ext] : 'unknown';
	...
	protected function copy($src, $dst, $name) {
		...
		if (!empty($srcStat['thash'])) {
			...
			return $stat && $this->_symlink($target, $dst, $name)
				...
				: $this->setError(elFinderUG::ERROR_COPY, $this->_path($src));
		} 
		...
		if ($srcStat['mime'] == 'directory') {
			...
			if (($test && $test['mime'] != 'directory') || !$this->_mkdir($dst, $name)) {
				return $this->setError(elFinderUG::ERROR_COPY, $this->_path($src));
			}
	...
	protected function move($src, $dst, $name) {
		...
		return $this->setError(elFinderUG::ERROR_MOVE, $this->_path($src));
	}
	...
	protected function copyFrom($volume, $src, $destination, $name) {
		...
		if (($source = $volume->file($src)) == false) {
			return $this->setError(elFinderUG::ERROR_COPY, '#'.$src, $volume->error());
		}
		
		$errpath = $volume->path($src);
		
		if (!$this->nameAccepted($source['name'])) {
			return $this->setError(elFinderUG::ERROR_COPY, $errpath, elFinderUG::ERROR_INVALID_NAME);
		}
		...
		if ($source['mime'] == 'directory') {
			...
			if ((!$stat || $stat['mime'] != 'directory') && !$this->_mkdir($destination, $name)) {
				return $this->setError(elFinderUG::ERROR_COPY, $errpath);
			}
	...
	protected function remove($path, $force = false) {
		...
		if (empty($stat)) {
			return $this->setError(elFinderUG::ERROR_RM, $this->_path($path), elFinderUG::ERROR_FILE_NOT_FOUND);
		}
		...
		if (!$force && !empty($stat['locked'])) {
			return $this->setError(elFinderUG::ERROR_LOCKED, $this->_path($path));
		}
		...
		if ($stat['mime'] == 'directory') {
			...
			if (!$this->_rmdir($path)) {
				return $this->setError(elFinderUG::ERROR_RM, $this->_path($path));
			}
