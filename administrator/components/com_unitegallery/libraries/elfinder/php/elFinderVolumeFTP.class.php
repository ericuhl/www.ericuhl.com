 ...
 **/
class elFinderVolumeFTPUG extends elFinderVolumeDriverUG {
	...
	protected function _extract($path, $arc)
	{
		...
		unlink($basename);
		$filesToProcess = elFinderVolumeFTPUG::listFilesInDirectory($tmpDir, true);
		if(!$filesToProcess) {
			$this->setError(elFinderUG::ERROR_EXTRACT_EXEC, $tmpDir." is not a directory");
		...
		if (!$success) {
			$this->setError(elFinderUG::ERROR_FTP_UPLOAD_FILE, $newPath);
			return false;
		}
		$this->clearcache();
		return $newPath;
	...
	protected function _archive($dir, $files, $name, $arc)
	{
		...
		if (!ftp_put($this->connect, $remoteArchiveFile, $path, FTP_BINARY)) {
			$this->setError(elFinderUG::ERROR_FTP_UPLOAD_FILE, $remoteArchiveFile);
			$this->deleteDir($tmpDir); //cleanup
			return false;
		}
	...
	private function tempDir()
	{
		...
		if (!$tempPath) {
			$this->setError(elFinderUG::ERROR_CREATING_TEMP_DIR, $this->tmp);
			return false;
		}
	...
	private function ftp_download_files($remote_directory, array $files, $dest_local_directory)
	{
		...
		if (!isset($contents)) {
			$this->setError(elFinderUG::ERROR_FTP_DOWNLOAD_FILE, $remote_directory);
			return false;
		}
	...
	private function deleteDir($dirPath)
	{
		if (!is_dir($dirPath)) {
			...
			$success = true;
			foreach (array_reverse(elFinderVolumeFTPUG::listFilesInDirectory($dirPath, false)) as $path) {
		...
		if(!$success) {
			$this->setError(elFinderUG::ERROR_RM, $dirPath);
			return false;
		}
		return $success;
	}
		...
		foreach($files as $file) {
			if(!in_array($file, $excludes)) {
				...
				if(is_link($path)) {
					...
					$subs = elFinderVolumeFTPUG::listFilesInDirectory($path, $omitSymlinks, $prefix.$file.DIRECTORY_SEPARATOR);
					if($subs) {
	...
	public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0) {
		...
		if ($result) {
			...
			if (!ftp_put($this->connect, $path, $local_path, FTP_BINARY)) {
				$this->setError(elFinderUG::ERROR_FTP_UPLOAD_FILE, $path);
				$this->deleteDir($tmpDir); //cleanup
			}
		...
		$this->setError(elFinderUG::ERROR_UNKNOWN);
		return false;
	}
